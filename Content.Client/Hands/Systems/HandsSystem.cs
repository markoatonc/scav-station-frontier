    ...
    public sealed class HandsSystem : SharedHandsSystem
    {
        ...
        private void UpdateHandVisuals(Entity<HandsComponent?, SpriteComponent?> ent, EntityUid held, string handId)
        {
            ...
            foreach (var (key, layerData) in ev.Layers)
            {
                ...
                if (handComp.HandDisplacements.TryGetValue(hand.Value.Location, out var disp))
                {
                    if (_displacement.TryAddDisplacement(disp, (ent, sprite), index, key, out var displacementKey))
